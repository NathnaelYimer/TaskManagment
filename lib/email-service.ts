...
export class EmailService {
  async sendTaskReminder(to: string, data: TaskReminderEmail) {
    const template = await this.getTemplate('task-reminder', data)
  ...
  async sendTaskAssignment(to: string, data: TaskAssignmentEmail) {
    const template = await this.getTemplate('task-assignment', data)
    return this.sendEmail(to, template.subject, template.html, template.text)
  }

  async sendWeeklyReport(to: string, data: WeeklyReportEmail) {
    const template = await this.getTemplate('weekly-report', data)
    return this.sendEmail(to, template.subject, template.html, template.text)
  }

  private async getTemplate(name: string, data: any): Promise<EmailTemplate> {
    const templates = {
      'task-reminder': {
